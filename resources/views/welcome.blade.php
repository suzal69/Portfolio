    ...
    <section class="pt-24 sm:pt-28 lg:pt-36 pb-16 sm:pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 lg:gap-12 items-center">
        ...
        <div class="lg:col-span-5 relative flex justify-center">
            <div class="w-full max-w-md glass-panel border border-slate-800/70 rounded-[2.5rem] p-6 shadow-2xl relative overflow-hidden text-center">
                ...
                <div class="w-40 h-40 sm:w-48 sm:h-48 mx-auto mb-6 rounded-[2rem] overflow-hidden border border-slate-700/70 bg-slate-900 shadow-inner">
    ...
    <section id="about" class="py-16 sm:py-20 lg:py-24 border-t border-slate-800/60 bg-[#0f1724]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 lg:gap-12 items-center">
            <div class="lg:col-span-6 space-y-4">
                ...
                <h2 class="text-3xl sm:text-4xl font-extrabold tracking-tight text-white">I build systems, interfaces, and clean backend workflows.</h2>
    ...
    <main id="projects" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
            @forelse($items as $item)
                <div class="group relative overflow-hidden rounded-[2.5rem] border border-slate-700/70 bg-slate-950/80 p-6 shadow-[0_20px_80px_-40px_rgba(14,165,233,0.5)] transition duration-500 hover:-translate-y-1 hover:border-sky-500/30 hover:bg-slate-900/95">
                    ...
                    <div class="relative z-10 space-y-6">
                        <div class="space-y-4">
                            <div class="inline-flex items-center gap-2 rounded-full bg-slate-900/80 px-4 py-2 text-xs uppercase tracking-[0.32em] text-sky-300 shadow-inner shadow-slate-950/50">
                                <span>Featured</span>
                            </div>
                            <h4 class="text-2xl sm:text-3xl font-black text-white tracking-tight leading-tight">{{ $item->title }}</h4>
                        </div>

                        <div class="flex flex-wrap justify-center gap-2 px-3 py-3 bg-slate-900/80 border border-slate-700/70 rounded-[1.75rem] shadow-inner">
                            @foreach(explode(',', $item->tech_stack) as $tech)
                                <span class="px-3 py-1 text-[10px] sm:text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-200 bg-slate-950/70 border border-slate-700 rounded-full">
                                    {{ trim($tech) }}
                                </span>
                            @endforeach
                        </div>
                    ...
                    @if($item->image_path)
                        <div class="mt-6 overflow-hidden rounded-[1.75rem] border border-slate-800/70 bg-slate-950 shadow-inner transition duration-700 group-hover:scale-[1.01]">
                            <img src="{{ asset('storage/' . $item->image_path) }}" alt="{{ $item->title }}" class="w-full h-56 object-cover" />
                        </div>
                    @endif

                    <div class="mt-6 flex flex-wrap gap-4 items-center border-t border-slate-800/70 pt-4 text-sm font-medium text-slate-300">
                        @if($item->live_url)
                            <a href="{{ $item->live_url }}" target="_blank" class="inline-flex items-center gap-2 rounded-full bg-sky-500/10 px-4 py-2 text-sky-300 transition hover:bg-sky-500/20 hover:text-white">
                                Live Demo <span class="text-xl">→</span>
                            </a>
                        @endif
                        @if($item->github_url)
                            <a href="{{ $item->github_url }}" target="_blank" class="inline-flex items-center gap-2 rounded-full bg-slate-900/80 px-4 py-2 text-slate-300 transition hover:bg-slate-800 hover:text-white">
                                GitHub
                            </a>
                        @endif
                    </div>
