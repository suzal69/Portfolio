                        ...
                        <div>
                            ...
                            @if($item->image_path)
                                <div class="overflow-hidden rounded-[1.3rem] border border-slate-800/50 bg-slate-950 shadow-inner mb-5">
                                    @php $imageUrl = Str::startsWith($item->image_path, 'http') ? $item->image_path : asset('storage/' . $item->image_path); @endphp
                                    <img src="{{ $imageUrl }}" alt="{{ $item->title }}" class="h-48 w-full object-cover transition-transform duration-700 ease-out group-hover:scale-105" />
                                </div>
                            @else
